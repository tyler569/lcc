...
size_t make_integer_token(Token* t, char* st, Location loc)
{
    ...
    if (st[0] == '\'') {
        ...
        if (st[1] == '\\') {
            if (st[3] != '\'') {
                lcc_error(loc, "expected closing \"'\".");
            }
            index = 4;
            switch (st[2]) {
            case 'n':
        ...
        if (base != 10) {
            lcc_error(loc, "I don't support non-base 10 yet, sorry");
        }
...
void debug_print_token(Token* t)
{
    ...
    case token_integer:
        printf("[Integer(%lu) @ %s:%lu:%lu]", t->value.integer, t->loc.filename,
            t->loc.line, t->loc.index);
        break;
    case token_string:
        printf("[String(%s) @ %s:%lu:%lu]", t->value.string, t->loc.filename,
            t->loc.line, t->loc.index);
        break;
    case token_ident:
        printf("[Ident(%s) @ %s:%lu:%lu]", t->value.ident, t->loc.filename,
            t->loc.line, t->loc.index);
        break;
    default:
...
{
    Location loc = { .filename = filename, 1, 1 };
    size_t index;
    size_t program_length = strlen(program);
    size_t tmp_len;
    Vector* tokens = new_vec(Token);
    ...
    for (index = 0; index < program_length;) {
        ...
        if (program[index] == '\n') {
            ...
            tmp_len = make_integer_token(&tmp, program + index, loc);
            ...
            lcc_warning(loc, "Numbers are a warning for testing");
